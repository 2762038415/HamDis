...
def main():
    ...
    parser_hamdist.add_argument("-v", dest="vmr", default="hmtop", help="区域筛选类型，默认 hmtop")
    parser_hamdist.add_argument("-vb", dest="vmr_bed", default=None, help="methscan/humb_old 模式下的输入 BED")
    parser_hamdist.add_argument("-hr", type=float, default=0.5, help="兼容原脚本的参数，占位保留")
    parser_hamdist.add_argument("-meta", default=None, help="兼容原脚本的参数，占位保留")
    parser_hamdist.add_argument("-pd", default=None, help="兼容原脚本的参数，占位保留")

    parser_hamdist.add_argument("--binsize", type=int, default=5000, help="bin size")
    parser_hamdist.add_argument("--seed", type=int, default=None, help="可选固定随机种子；默认保持原脚本非固定随机抽样")
    parser_hamdist.add_argument("--min_hm_count", type=int, default=5, help="构建 hmdistance mean matrix 的阈值")
    parser_hamdist.add_argument("--min_cpg", type=int, default=2, help="bin 内最少 CpG 数")
    parser_hamdist.add_argument("--max_dis", type=int, default=600, help="read 回溯距离")
    parser_hamdist.add_argument("--max_possible_read", type=int, default=3000, help="每个 bin 最大 read 数")
