...
def sample_pat(pat_dir, chr_list, batchsize, batchnum, merge_root, seed=None):
    """
    对齐 pipeline-v9/2-hamDist.py 的采样逻辑：
    - 只从 chr_list[0] 读取一次 pat 名称
    - 每个 batch 都在同一个 pat_names 列表上做一次原地 shuffle
    - 若 seed 为 None，则保持原脚本的非固定随机行为
    - 输出 merge_pat_record.txt，方便和原流程一样追踪抽样结果
    """
    if not chr_list:
        return [], {}
    ...
    for _ in range(batchnum):
        random.shuffle(pat_names)
        sampled_samples_batch = pat_names[:min(batchsize, len(pat_names))]
        sampled_samples.append(sampled_samples_batch)

    safe_makedirs(merge_root)
    record_file = os.path.join(merge_root, "merge_pat_record.txt")

    sampled_files_map = {}
    merge_files = []
    with open(record_file, "a+") as record_handle:
        for chr_name in chr_list:
            for bi, sampled_batch in enumerate(sampled_samples):
                ...
                if (not os.path.exists(out_pat)) or os.path.getsize(out_pat) == 0:
                    record_handle.write(f"{out_pat}: {sampled_files}\n")
...
def pat_to_Hamming(merge_pat_file, cpg_file, region_file, out_file,
                   binsize=5000, basedon0=0, min_cpg=2, max_dis=600,
...
def build_hmdistance_mean_matrix(metrics_dir, output_file, min_hm_count=5):
    """
    对齐 pipeline-v9/scripts/get_hmdistance_mean_matrix_args_new.sh：
    - 使用 chr*_merge_0_MeConcord.txt 的第2/3/4列作为模板
    - 依次横向追加 chr*_*.txt 的 HammingSum/HammingCount 比值
    - HammingCount < threshold 时记为 NaN
    """
    files = sorted(glob.glob(os.path.join(metrics_dir, "*_MeConcord.txt")))
    if not files:
        pd.DataFrame(columns=["chrom", "start", "end"]).to_csv(output_file, sep="\t", index=False)
        return output_file

    merge0_file = None
    for fp in files:
        if os.path.basename(fp).endswith("merge_0_MeConcord.txt"):
            merge0_file = fp
            break
    if merge0_file is None:
        merge0_file = files[0]

    base_df = pd.read_csv(merge0_file, sep="\t")
    base = base_df.iloc[:, 1:4].copy()

    for fp in files:
        file_basename = os.path.basename(fp)
        title = file_basename
        if title.startswith(f"{os.path.basename(metrics_dir)}_"):
            title = title[len(os.path.basename(metrics_dir)) + 1:]
        title = f"{title}_min{min_hm_count}"

        df = pd.read_csv(fp, sep="\t")
        hamming_count = pd.to_numeric(df.iloc[:, 10], errors="coerce")
        hamming_sum = pd.to_numeric(df.iloc[:, 11], errors="coerce")
        ratio = hamming_sum / hamming_count
        ratio.loc[hamming_count < min_hm_count] = np.nan
        ratio = ratio.reset_index(drop=True)

        if len(ratio) < len(base):
            ratio = ratio.reindex(range(len(base)))
        else:
            ratio = ratio.iloc[:len(base)]
        base[title] = ratio.values

    base.to_csv(output_file, sep="\t", index=False)
    return output_file


def stat_hmdistance_mean_matrix(mean_matrix_file, stat_file):
    """
    对齐 pipeline-v9/scripts/stat_hmdistance_mean_matrix_args.py：
    - 只保留前三列位置列
    - 使用第4列之后的数值列计算 Mean/Sum/CV/Sum_CV/SD
    - 不输出 ValidN
    """
    data = pd.read_csv(mean_matrix_file, sep="\t")
    position_cols = data.iloc[:, :3].copy()
    data_cols = data.iloc[:, 3:].apply(pd.to_numeric, errors="coerce")

    results = position_cols.copy()
    results["Chromosome"] = data.iloc[:, 0]
    means = []
    sum_values = []
    cv_values = []
    sum_cv_values = []
    ...
    for _, row in data_cols.iterrows():
        ...
        n_valid = valid_values.sum()
        n_total = len(row)

        if n_total == 0 or n_valid / n_total <= 0.8:
            means.append(np.nan)
            sum_values.append(np.nan)
            cv_values.append(np.nan)
            sum_cv_values.append(np.nan)
            sds.append(np.nan)
            continue

        valid_data = row[valid_values].values
        ...
        if mean_val == 0:
            ...
            sd_val = np.std(valid_data, ddof=1)
            cv = sd_val / mean_val

        means.append(mean_val)
        sum_values.append(row_sum)
        cv_values.append(cv)
        sum_cv_values.append(row_sum * cv if not np.isnan(cv) else np.nan)
        sds.append(sd_val)

    results["Mean"] = means
    results["Sum"] = sum_values
    results["CV"] = cv_values
    results["Sum_CV"] = sum_cv_values
    results["SD"] = sds
    results.to_csv(stat_file, sep="\t", index=False)
    return stat_file


def get_stat_value(metrics_root, hm_dir, chr_list, min_hm_count=5):
    safe_makedirs(hm_dir)
    all_stat_dfs = []

    for chr_name in chr_list:
        chr_num = process_chr_name(chr_name)
        if not chr_num:
            print(f"[WARN] 跳过无效染色体: {chr_name}")
            continue
        ...
        mean_matrix_file = os.path.join(hm_dir, f"hmdistance_mean_chr{chr_num}.txt")
        stat_file = os.path.join(hm_dir, f"hmdistance_stat_chr{chr_num}.tsv")

        build_hmdistance_mean_matrix(chr_metrics_dir, mean_matrix_file, min_hm_count=min_hm_count)
        stat_hmdistance_mean_matrix(mean_matrix_file, stat_file)

        try:
            sdf = pd.read_csv(stat_file, sep="\t")
        except Exception as e:
            print(f"[WARN] 读取统计文件失败，跳过 {stat_file}: {e}")
            continue

        if sdf.shape[0] > 0:
            sdf["chr_num"] = sdf["Chromosome"].astype(str).str.extract(r"(\d+)")
            sdf.loc[sdf["chr_num"].notna(), "chr_num"] = sdf.loc[sdf["chr_num"].notna(), "chr_num"].astype(int)
            all_stat_dfs.append(sdf)

    merged_stat = os.path.join(hm_dir, "hmdistance_stat_allchr.tsv")
    if all_stat_dfs:
        combined = pd.concat(all_stat_dfs, ignore_index=True)
        combined.sort_values(["chr_num"] + list(combined.columns[:3]), inplace=True)
        combined.drop(columns=["chr_num"], inplace=True)
        combined.to_csv(merged_stat, sep="\t", index=False)
    else:
        pd.DataFrame(columns=["chrom", "start", "end", "Chromosome", "Mean", "Sum", "CV", "Sum_CV", "SD"]).to_csv(
            merged_stat, sep="\t", index=False
        )
    return merged_stat


def extract_ref_cpg(interval_file, cpg_file_path, output_prefix, output_folder, chr_list, size):
    """
    对齐 pipeline-v9/scripts/get_ref_cpg_args.py：
    - 端点使用闭区间 [start, end]
    - 不做去重
    - 各染色体分别输出，再由上层按文件名拼接
    """
    ...
    with open(interval_file, "r") as handle:
        for line in handle:
            ...
            if len(parts) < 3:
                continue
            if (not parts[0].startswith("chr")) or parts[0] == "chrom":
                continue
            intervals.append({"chr": parts[0], "start": int(float(parts[1])), "end": int(float(parts[2]))})
    ...
    for chr_name in chr_list:
        ...
        cpg_df = pd.read_csv(cpg_file, sep="\t", header=None, names=["chr", "start", "order"])
        filtered_intervals = [x for x in intervals if x["chr"] == chr_name]
        if not filtered_intervals:
            continue

        merged_data = pd.DataFrame(columns=["chr", "start", "order"])
        for interval in filtered_intervals:
            filtered = cpg_df[
                (cpg_df["chr"] == chr_name) &
                (cpg_df["start"] >= interval["start"]) &
                (cpg_df["start"] <= interval["end"])
            ]
            merged_data = pd.concat([merged_data, filtered], axis=0)

        out_file = os.path.join(output_folder, f"{output_prefix}{size}_{chr_name}.txt")
        merged_data.to_csv(out_file, sep="\t", index=False, header=False)
        produced.append(out_file)

    return produced


def run_commands_1(size, hm_dir, stat_filename, diff_result, cpg_file_dir, chr_list, vmr="hmtop", vmr_bed=None):
    """
    对齐 pipeline-v9/2-hamDist.py 的 run_commands_1：
    - hmtop/hmrand/hmlow/hmlowprod 均保留原始 shell 逻辑
    - 原始脚本对 hmtop 排序使用的是第9列；这里保持这一行为不变
    """
    safe_makedirs(diff_result)
    stat_file = os.path.join(hm_dir, stat_filename)

    if "hm" in vmr:
        output_prefix = f"SD_{vmr}{size}"
        interval_file = os.path.join(diff_result, f"{output_prefix}.bed")
        if vmr == "hmtop":
            cmd = (
                f"sort -k9,9gr {stat_file} | head -\"{size}\" | "
                f"sort -k1,1 -k2,2n | awk '{{print $1\"\\t\"$2\"\\t\"$3}}' > {interval_file}"
            )
        elif vmr == "hmrand":
            cmd = (
                f"shuf -n {size} {stat_file} | "
                f"sort -k1,1 -k2,2n | awk '{{print $1\"\\t\"$2\"\\t\"$3}}' > {interval_file}"
            )
        elif vmr == "hmlow":
            cmd = (
                f"sort -k9,9g {stat_file} | head -\"{size}\" | "
                f"sort -k1,1 -k2,2n | awk '{{print $1\"\\t\"$2\"\\t\"$3}}' > {interval_file}"
            )
        elif vmr == "hmlowprod":
            cmd = (
                f"sort -k8,8g {stat_file} | head -\"{size}\" | "
                f"sort -k1,1 -k2,2n | awk '{{print $1\"\\t\"$2\"\\t\"$3}}' > {interval_file}"
            )
        else:
            raise ValueError(f"不支持的 vmr 类型: {vmr}")
        subprocess.run(cmd, shell=True, check=True)
    elif vmr == "methscan":
        interval_file = vmr_bed
        size = -1
    elif vmr == "humb_old":
        interval_file = "/home/lijia/yanhongliang/proj/pipeline-v9/middle_res/humbv9/40_40/diff_result/SD_hmtop40000.bed"
        size = 40000
    else:
        raise ValueError(f"不支持的 vmr 类型: {vmr}")

    output_prefix = f"cpg_pos_{vmr}"
    produced = extract_ref_cpg(
        interval_file=interval_file,
        cpg_file_path=cpg_file_dir,
        output_prefix=output_prefix,
        output_folder=diff_result,
        chr_list=chr_list,
        size=size,
    )

    cpg_pos = os.path.join(diff_result, f"cpg_pos_{vmr}{size}_allchr.txt")
    with open(cpg_pos, "w") as fout:
        for fp in sorted(produced):
...
def run_hamdist(
    ...
    hm_dir = os.path.join(diff_result, f"hm_{bin_num}")
    ...
    stat_filename = os.path.basename(
        get_stat_value(metrics_root, hm_dir, chr_list, min_hm_count=min_hm_count)
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="按 pipeline-v9/total_batch_xunyin2.sh 规则运行 hamdist"
    )
    ...
    parser.add_argument("-v", "--vmr", default="hmtop", help="区域筛选类型，默认 hmtop")
    parser.add_argument("-vb", "--vmr_bed", default=None, help="methscan/humb_old 模式下的输入 BED")
    parser.add_argument("--binsize", type=int, default=5000, help="pat_to_Hamming 划分 bin 大小")
    parser.add_argument("--seed", type=int, default=None, help="可选固定随机种子；默认保持原脚本非固定随机抽样")
    parser.add_argument("--min_hm_count", type=int, default=5, help="原始 get_hmdistance_mean_matrix 阈值")
    parser.add_argument("--min_cpg", type=int, default=2, help="原始 pat_to_Hamming 的 min_cpg")
    parser.add_argument("--max_dis", type=int, default=600, help="原始 pat_to_Hamming 的 max_dis")
    parser.add_argument("--max_possible_read", type=int, default=3000, help="原始 pat_to_Hamming 的 max_possible_read")
